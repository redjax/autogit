...
@nox.session(python=[DEFAULT_PYTHON], name="build-pex")
def build_pex_file(session: nox.Session):
    session.install("pex")

    output_pex: Path = Path("./dist/autogit.pex")
    if not output_pex.exists():
        if not output_pex.parent.exists():
            try:
                output_pex.parent.mkdir(exist_ok=True, parents=True)
            except Exception as exc:
                msg = Exception(
                    f"Unhandled exception creating .pex file output dir '{output_pex}'. Details: {exc}"
                )

                raise msg

    print(f"Installing to {output_pex}")
    try:
        session.run(
            f"pex -r requirements/requirements.txt -o ./{output_pex} -- ./src/autogit/main.py"
        )
        print(f".pex built successfully, output: {output_pex}")
    except Exception as exc:
        msg = Exception(f"Unhandled exception building .pex file. Details: {exc}")
